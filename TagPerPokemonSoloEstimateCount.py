...
def create_pokemon_list(base,target_tags):
	data_list=[]
	for pokemon in base:
		tempdict={"name":pokemon,"total_count":0,"solo_count":0}
	...
	for pokemon in data_list:
			...
			while next_page:
				...
				if tag_count!=0:
					for post in response.json()["posts"]:
						pokemon["solo_count"]=pokemon["solo_count"]+1
						tag_list=[]
		...
		if default_tag:
			...
			while next_page:
				...
				time.sleep(1)
				#gets 320 posts from given page with tags "-rating:safe" and the pokemon.
				response=requests.get("https://e621.net/posts.json?page="+str(page)+"&tags=-rating%3As+~"+pokemon["name"]+"+~alolan_"+pokemon["name"]+"+~galarian_"+pokemon["name"]+"+~mega_"+pokemon["name"]+"+~gigantamax_"+pokemon["name"]+"&limit=320",params=params,headers=headers)
				if str(response)!="<Response [200]>": #error test
					print(response)
				tag_count=len(response.json()["posts"])
			...
			while next_page:
				...
				time.sleep(1)
				#gets 320 posts from given page with tags "-rating:safe" and the pokemon, but with "_(pokémon)" added to the end to prevent ambiguity. i.e. gloom
				response=requests.get("https://e621.net/posts.json?page="+str(page)+"&tags=-rating%3As+~"+pokemon["name"]+"_(pokémon)+~alolan_"+pokemon["name"]+"+~galarian_"+pokemon["name"]+"+~mega_"+pokemon["name"]+"+~gigantamax_"+pokemon["name"]+"&limit=320",params=params,headers=headers)
				if str(response)!="<Response [200]>": #error test
					print(response)
				tag_count=len(response.json()["posts"])
				total+=tag_count #add number of posts on this page to overall total
		...
		pokemon["total_count"]=total #set the number of posts of the pokemon to the number of posts found via this part of the code

		
	#sort list by most tagged and return
	data_list=sorted(data_list, key=lambda pokemon: pokemon["total_count"],reverse=True)
	return data_list
...
def save_data(data_list,date_string,target_tags):
	...
	with open('tagPerPokemonSoloEstimateCount'+date_string+'.csv', 'w', newline='') as csvfile:
		sheet=csv.writer(csvfile)
		#first line:
		row_list=["Rank","Pokemon","Count","Solo Count","Error Potential"]
		for tag in target_tags:
				row_list.append("#"+tag)
				row_list.append("%"+tag)
		...
		for pokemon in data_list:
			...
			if pokemon["solo_count"]!=0:
				row_list=[str(i),pokemon["name"],str(pokemon["total_count"]),str(pokemon["solo_count"]),str(100*pokemon["solo_count"]/pokemon["total_count"])+"%"]
				for tag in target_tags:
					row_list.append(str(int(pokemon["total_count"]*pokemon[tag]/pokemon["solo_count"])))
					row_list.append(str(round(100*pokemon[tag]/pokemon["solo_count"],2))+"%")
				sheet.writerow(row_list)
			else:
				empty_pokemon_list.append(pokemon["name"])
				
	print("Done!")
	print("Pokémon with no solo entries:")
	for pokemon in empty_pokemon_list:
		print(pokemon)
